...
class AsistenciaService:
    ...
    def registrar_entrada(self, documento):
        ...
        if not self.db.cursor.fetchone():
            return False, "❌ El aprendiz NO existe o está en papelera."

        # Validar si ya tiene una entrada abierta
        ...
        # Insertar (Competencia 1 por defecto)
    ...
    def actualizar_password(self, documento, nueva_pass):
        """Actualiza la contraseña y marca el primer cambio como realizado."""
        if len(nueva_pass) < 4:
    ...
    def obtener_fichas(self):
        """Carga las fichas disponibles para el Combobox."""
    ...
    def guardar_aprendiz_manual(self, datos, id_ficha):
        """Guarda un nuevo aprendiz validando campos."""
        if "" in datos.values() or not id_ficha:
            return False, "Todos los campos son obligatorios."
        try:
            self.db.cursor.execute(
                "INSERT INTO estudiantes (documento, nombre_completo, correo, id_ficha) VALUES (%s, %s, %s, %s)", 
                (datos["Documento"], datos["Nombre Completo"], datos["Correo"], id_ficha)
            )
            self.db.conexion.commit()
            return True, "✅ Aprendiz guardado."
        except Exception as e:
            return False, f"Error: {str(e)}"

    def mandar_a_papelera(self, documento):
        """Mueve un estudiante a la tabla de eliminados."""
        try:
            self.db.cursor.execute(
    ...
    def restaurar_aprendiz(self, documento):
        """Restaura un estudiante desde la papelera."""
        try:
            self.db.cursor.execute(
    ...
    def eliminar_permanente(self, documento):
        """Borra definitivamente de la papelera."""
        try:
            self.db.cursor.execute("DELETE FROM estudiantes_eliminados WHERE documento=%s", (documento,))
            self.db.conexion.commit()
            return True
    ...
    def obtener_registros_dia(self, documento, fecha):
        """Obtiene las entradas y salidas de un aprendiz en una fecha específica."""
