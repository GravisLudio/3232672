...
class SistemaHSGS:
    ...
    def mostrar_panel_admin(self):
        ...
        tv_gest = tv_gest = ttk.Treeview(t_gest, columns=("DOC", "NOM", "FICHA"), show="headings", selectmode="extended")
        ...
        def mandar_papelera():
            selecciones = tv_gest.selection() # Obtiene todos los IDs seleccionados
            if not selecciones:
                messagebox.showwarning("HSGS", "Por favor, seleccione al menos un aprendiz.")
                return
            
            confirmar = messagebox.askyesno("HSGS", f"¿Mover {len(selecciones)} aprendiz(ces) a la papelera?")
            if confirmar:
                try:
                    for sel in selecciones:
                        # Obtener el documento de la fila actual
                        ...
                        self.db.cursor.execute("""
                            ...
                            FROM estudiantes WHERE documento=%s
                        """, (doc,))
                    ...
                    messagebox.showinfo("HSGS", f"✅ {len(selecciones)} registros movidos con éxito.")
                    filtrar()      # Refrescar tabla gestión
                    refresh_pap()  # Refrescar tabla papelera
        ...
        cb_ficha = ttk.Combobox(f1, state="readonly", width=45) 
        cb_ficha.grid(row=1, column=3, padx=5, sticky="ew")

        def actualizar_fichas():
            # Solo traemos ID y Nombre del Programa de la base de datos
            self.db.cursor.execute("SELECT id_ficha, nombre_programa FROM fichas")
            fichas = self.db.cursor.fetchall()
            # Formato más limpio: "ID | NOMBRE"
            cb_ficha['values'] = [f"{f['id_ficha']} | {f['nombre_programa']}" for f in fichas]
        ...
        def save_one():
            ...
            try:
                ...
                self.db.conexion.commit()
                messagebox.showinfo("HSGS", "✅ Aprendiz registrado correctamente.")
                ...
                # Extraemos el código de error de la excepción
                if hasattr(e, 'errno') and e.errno == 1062:
                    messagebox.showerror("HSGS - Error de Registro", 
                                        f"❌ El documento '{vals['Documento']}' ya se encuentra registrado en el sistema.\n\nPor favor, verifique los datos o busque al aprendiz en la pestaña Gestión.")
                else:
                    messagebox.showerror("HSGS - Error Crítico", f"Ocurrió un error inesperado:\n{str(e)}")
        ...
        tv_pap = ttk.Treeview(t_pap, columns=("DOC", "NOM", "FICHA"), show="headings", selectmode="extended")
        ...
        def restaurar():
            selecciones = tv_pap.selection()
            if selecciones and messagebox.askyesno("HSGS", f"¿Restaurar {len(selecciones)} aprendices?"):
                for sel in selecciones:
        ...
        def hard_delete():
            selecciones = tv_pap.selection()
            if selecciones and messagebox.askyesno("HSGS CRÍTICO", f"¿Borrar PERMANENTEMENTE {len(selecciones)} registros?"):
                for sel in selecciones:
    ...
    def actualizar_password_ventana(self, documento):
        ...
        v.title("Seguridad Requerida")
        v.geometry("450x450")
        v.configure(bg="white")
        v.transient(self.root)
        v.grab_set()
        
        v.protocol("WM_DELETE_WINDOW", lambda: messagebox.showwarning("Seguridad", "Debe actualizar su contraseña para continuar."))

        tk.Label(v, text="🔒 ACTUALIZACIÓN OBLIGATORIA", font=("Segoe UI", 12, "bold"), bg="white", fg="#d32f2f").pack(pady=10)
        
        # Variable para rastrear el texto
        pass_var = tk.StringVar()
        e = ttk.Entry(v, show="*", textvariable=pass_var, font=("Segoe UI", 12))
        e.pack(pady=10, padx=40, fill="x")
        e.focus()

        # Contenedor de requisitos
        req_frame = tk.Frame(v, bg="white")
        req_frame.pack(pady=10, padx=40, fill="x")

        # Diccionario para gestionar las etiquetas de requisitos
        self.requisitos = {
            "long":  {"txt": "• Mínimo 8 caracteres", "lbl": tk.Label(req_frame, text="• Mínimo 8 caracteres", bg="white", fg="red", anchor="w")},
            "upper": {"txt": "• Al menos una mayúscula", "lbl": tk.Label(req_frame, text="• Al menos una mayúscula", bg="white", fg="red", anchor="w")},
            "lower": {"txt": "• Al menos una minúscula", "lbl": tk.Label(req_frame, text="• Al menos una minúscula", bg="white", fg="red", anchor="w")},
            "num":   {"txt": "• Al menos un número", "lbl": tk.Label(req_frame, text="• Al menos un número", bg="white", fg="red", anchor="w")},
            "spec":  {"txt": "• Al menos un caracter especial (@$!%*?&)", "lbl": tk.Label(req_frame, text="• Al menos un caracter especial", bg="white", fg="red", anchor="w")}
        }

        for r in self.requisitos.values():
            r["lbl"].pack(fill="x")

        def validar_tiempo_real(*args):
            p = pass_var.get()
            ...
            for clave, cumple in condiciones.items():
                if cumple:
                    self.requisitos[clave]["lbl"].config(fg="#39A900", text=self.requisitos[clave]["txt"] + " ✓")
                else:
                    self.requisitos[clave]["lbl"].config(fg="red", text=self.requisitos[clave]["txt"])
                    todo_valido = False
            
            return todo_valido

        # Vincular la validación al escribir
        pass_var.trace_add("write", validar_tiempo_real)

        def save():
            if validar_tiempo_real():
                if pass_var.get() == "sena123":
                    messagebox.showwarning("HSGS", "No puede usar la clave por defecto.")
                    return
                try:
                    query = "UPDATE estudiantes SET password=%s, cambio_pass=1 WHERE documento=%s"
                    self.db.cursor.execute(query, (pass_var.get(), documento))
                    self.db.conexion.commit()
                    messagebox.showinfo("HSGS", "✅ Seguridad configurada. Bienvenido.")
                    v.grab_release()
                    v.destroy()
                except Exception as ex:
                    messagebox.showerror("Error", f"Error de base de datos: {ex}")
            else:
                messagebox.showwarning("HSGS", "Aún no cumples todos los requisitos.")

        btn_save = tk.Button(v, text="ACTUALIZAR Y ENTRAR", bg="#39A900", fg="white", font=("bold", 10), command=save)
        btn_save.pack(pady=20, ipadx=20)
        v.bind('<Return>', lambda event: save())
